...
# 把项目内 Skill 链接到 .claude/skills/，Claude Code 即可发现 /character-workflow、/viewer-server
...
dev-link:
	...
	@echo "Linked: .claude/skills/{character-workflow,viewer-server} → skill/*"
	@echo "Restart Claude Code session to pick up the new skills."

dev-unlink:
	rm -f .claude/skills/character-workflow .claude/skills/viewer-server
