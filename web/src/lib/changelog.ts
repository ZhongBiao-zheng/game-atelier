...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    changes: [
      ...
      { kind: 'fix', text: '首页、创作台输入框、画布生成面板与生成偏好弹窗不再显示「当前模型不支持原尺寸模式」「参考图最多 N 张」这类提示，尺寸与参考图数量按模型上限静默调整。' },
    ],
  },
  {
    version: '5.62.5',
  ...
  },
  {
    ...
    headline: '画布视频控制栏重排',
    changes: [
      { kind: 'feat', text: '视频节点控制栏按 播放 / 已播时间 / 进度条 / 剩余时间 / 音量 / 全屏 一排排列；音量改为悬停或聚焦音量图标时竖向弹出，不再挤占进度条。' },
    ],
  },
  {
    version: '5.62.3',
  ...
  },
  {
    ...
    changes: [
      ...
      { kind: 'fix', text: '上传 .mp4 / .mov 时，品牌为 QuickTime 的 mp4、ftyp 前带空白 box 的视频（飞书 / 微信转存常见）不再被判「内容与扩展名不一致」；被拒时报错会点名实际识别出的类型。' },
    ],
  },
  {
    version: '5.62.1',
  ...
  },
  {
    ...
    changes: [
      ...
      { kind: 'feat', text: '新增「本机 Agent 连接」页：按项目授权 Codex / Claude 通过 MCP 读取上下文、维护文档、准备生成；默认满能力，勾选「直接执行生成」后终端确认即出图，否则在「待批准生成」页人工批准；每条授权直接给出可粘贴的注册命令' },
      { kind: 'feat', text: '终端 Skill 的 CLI 出图流程保持不变；Agent 可读写出图经验，评分高的图仍会提醒沉淀' },
      { kind: 'feat', text: '画布也能授权给 Agent：读画布、增删节点与连线、填生成配置、导入本机文件；勾选「发起生成」后可直接在画布上出图' },
      { kind: 'feat', text: '启动器改为按运行实例握手，不再误判其他程序占用的端口；旧记录存活但无法核验时不会再开第二个服务' },
      { kind: 'feat', text: '创作资产里的提示词模板对 Agent 开放：按标签查索引、读全文填变量；模板可附「推荐配置」（模型 id + 参数），Agent 出图时按它定模型与参数' },
      { kind: 'feat', text: '画布图片工具栏新增「抠图」：本机运行 BiRefNet 模型自动去背景，生成透明底新节点；首次使用下载一次模型（约 214 MB），不上传图片；Intel Mac 暂不支持' },
      { kind: 'fix', text: '创作台未提交的提示词、参考素材与配置在切到设置等其他页面后保留，回来继续填；刷新页面才清空' },
