...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  },
  {
    ...
    changes: [
      { kind: 'feat', text: '团队库目录格式写成公开文档（docs/team-library-format.md），其他工具照它读写即可被识别、采用、复刻。' },
      { kind: 'fix', text: '同事的分享只在第一次可用时提醒一次；改标题、改标签不再弹。网盘分批同步完成那一刻才提醒。' },
      { kind: 'fix', text: '画布里分享时，团队库列表只显示挂在这块画布上的库；先关掉资产面板再点提醒条上的「看看」也能打开团队栏。' },
      { kind: 'fix', text: '本机资产库文件损坏时直接报「数据损坏」和原因，不再提示刷新重试；服务启动时多个团队库并行补扫，一个网盘卡住不影响其他库。' },
      { kind: 'fix', text: '生成任务的参考图只能来自数据目录（密钥与运行时目录除外）。引用了数据目录外文件的旧记录，「再次生成」会报错，需重新上传参考图。' },
    ],
  },
  {
    version: '5.66.0',
    date: '2026-09-24',
    headline: '团队库：画布也能分享和复刻，同事分享会提醒你',
    changes: [
      { kind: 'feat', text: '画布节点工具条加「保存」「分享」：生成出来的图 / 视频存成生成资产（带配方），也能直接分享到团队库。' },
      { kind: 'feat', text: '在画布里复刻：一键摆好参考图节点、生成配置节点和连线，模型参数照原配方，不自动运行。' },
      { kind: 'feat', text: '同事分享了新内容，右上角弹一条提醒，可以直接「复刻」或「看看」。' },
      { kind: 'feat', text: '采用过的资产，来源更新会显示「来源已更新」，点「重新采用」覆盖成最新版；来源撤回显示「来源已撤回」，本机副本保留。' },
      { kind: 'feat', text: '把团队库里的素材拖上画布，若同事的配方用过它，会提示「有 N 条相关配方」。' },
      { kind: 'feat', text: 'Studio 生成结果「保存为创作资产」现在存成生成资产，视频结果也能保存；团队栏视频显示首帧。' },
    ],
  },
  {
    version: '5.65.0',
    date: '2026-09-24',
    headline: '团队库：分享成片，一键复刻同事的配方',
    changes: [
      { kind: 'feat', text: 'Studio 历史里的每张图 / 每个视频多了「分享」：选团队库、改标题加标签，成片连同提示词、模型、参数和参考图一起写进团队目录。' },
      { kind: 'feat', text: '团队栏的配方卡可以「复刻」：提示词、模型、参数和当时的参考图直接填进输入框，由你自己点生成；输入框里已有内容会先问一句。本机没有那个模型时模型位留空并提示，不会偷偷换成别的模型。' },
      { kind: 'feat', text: '新资产种类「生成资产」：成片 + 生成配方，采用同事的配方后留在你的资产库里，随时再复刻。' },
      { kind: 'feat', text: '自己分享的内容可以在团队栏里改标题、标签或撤回；团队栏顶部列出最近的「相关配方」。' },
      { kind: 'fix', text: '图片 / 视频模式来回切换后，界面显示的模型就是实际提交的模型。' },
      { kind: 'fix', text: '提示词资产去掉「推荐模型」设置：要连模型和参数一起复用，请用生成资产。' },
    ],
  },
  {
    version: '5.64.0',
    ...
    changes: [
      { kind: 'feat', text: '团队库：在设置里给画布挂一个同步目录，资产面板多出「团队」一栏，能浏览同事放进去的图 / 视频 / 音频并采用到自己的资产库，或直接拖上画布。' },
      { kind: 'feat', text: '设置里可以填显示名，分享时用它标作者。' },
      { kind: 'fix', text: '「图片资产」改名「媒体资产」，现在也能存视频和音频。' },
      { kind: 'fix', text: '文本节点上的「生成图片」现在直接生成一个图片节点并打开生成面板，文本作为 @引用进提示词；不再先落一个配置节点。' },
      { kind: 'feat', text: '画布节点工具栏加「隐藏」：把节点内容模糊打码，标题和连线照常，再点「显示」恢复；状态随画布保存。' },
    ],
  },
  {
    version: '5.63.0',
