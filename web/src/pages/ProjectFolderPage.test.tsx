...
const folders = {
  folders: [{
    ...
    items: [
      ...
      { kind: 'ui_screen', asset_id: 'home', scheme_id: 'v1' },
...
beforeEach(() => {
  vi.stubGlobal('fetch', vi.fn(async (url: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/api/projects/p1/ui-schemes') return response({
      default_scheme_id: 'v1', schemes: [{ id: 'v1', name: 'V1', created_at: '' }],
    });
    if (path.startsWith('/api/projects/p1/workspaces')) return response(workspace);
    if (path === '/api/gallery/screens?project=p1&scheme=v1') return response({ items: [{
      screen_id: 'inventory', filename: 'v1.png', path: 'projects/one/ui/v1/screens/inventory/v1.png',
...
describe('ProjectFolderPage', () => {
  ...
  it('按视图过滤成员但不改变引用', async () => {
    renderPage('ui');
    expect(await screen.findByText('V1 · 主界面')).toBeInTheDocument();
    ...
    expect(screen.getByRole('button', { name: '添加UI 页面 V1 · inventory' })).toBeInTheDocument();
  });
