...
| 单页生成 / 基准页 / 页面生图 | `ui-page`（**未上线**，B2 批次交付——如实告知，别硬出） |
| 风格切换 / 风格候选对比 | `ui-page` 风格切换模式（**未上线**，B3 批次交付） |
...
| 3 | 基准页（ui-page，未上线） | `screens/<id>/v1.png` | 同上 + style.md 存在 |
| 4 | 风格定稿（ui-page 风格切换，未上线） | 候选对比 + screens canonical | 基准页结构经画师确认 |
| 5 | 页面延展（ui-screens，未上线） | `screens/screen-map.md` | 风格已定稿（style.md `ui.*` approved） |
| 6 | 逐页生成（ui-page，未上线） | `screens/<id>/vN.png` | screen-map 范围经画师批准 |
...
- 未上线阶段（ui-page / ui-screens）如实告知，不伪造产物、不用 Studio 自由出图冒充流程产物。
