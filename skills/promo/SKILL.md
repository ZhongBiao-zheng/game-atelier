...
version: 1.4.0
description: |
  为项目内已有角色出宣传图（KV、海报、美宣）或改已出美宣时用；需要 spec 且每个出镜角色有一张身份图（立绘、三视图或上传图）。立绘走 character，三视图走 turnaround；与项目角色无关的泛海报不用。
allowed-tools:
  - Bash
  - Read
  - Write
...
返回 `stage / recommend_action / active_id / spec / project_worldview / project_style / derivative / canonical / lessons_workspace / lessons_project`（出图经验来自 `<data_root>/MEMORY.md`，按 kind 分段，本 skill 取 promo 段；`project_style` 为项目风格契约全文，非空时其 `style` / `palette` / `taboo` 字段一并作 prompt 约束）。
...
**角色衍生**：`derivative` 非 null 时，仍只读写当前 `active_id` 的美宣、定稿、反馈和 job。优先引用当前衍生自己的定稿立绘；尚无立绘时按 `derivative.source_paths` 顺序使用创建时冻结的来源图，并结合当前 spec 表达本次延伸目标。
...
4. 渲染成品给画师：只用 run-job 返回 JSON 里的 `output_paths` 数组（本次 job 自己的字段），按序每张一行 Markdown 图片。客户端有 `show_widget` 时先跑 `card <job_id>`（job 已 done 会输出结果卡：缩略图 + 「vN 定稿 / 要改 / 先放着」按钮）渲染成卡片，再按下面的渲染通道把原图发给画师——卡片里只有缩略图，不替代原图（`docs/references/card-widget.md` 第四节）。图片地址按**渲染通道**选（**完整规则 + 判断方法 + 降级顺序见 `docs/references/image-presentation.md`**）：
...
本 Skill 的知识层（记忆注入、设定协议、prompt 规则、经验沉淀）与「手」无关。客户端工具列表里有 `workshop_*` 工具 → 走 MCP，CLI 命令与 MCP 工具的对应关系见 `docs/references/workshop-mcp-workflow.md`「连接与资料边界」；没有 → 走本文的 CLI 命令。同一轮绝不混用两条手。

批准门不随手变：确认卡 + 画师明确肯定。MCP 路径下持 `execute_generation` 者用 `workshop_approve_generation` 完成批准，不持有则请画师去 Atelier「待批准生成」页确认。
