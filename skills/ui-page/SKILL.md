...
多项目用 AskUserQuestion 选定，拿到 `project_slug`。随后 Read `ui/schemes.json`：用户点名 V1/V2
就选对应项，否则用 `default_scheme_id`；把 `scheme_id` 明示在确认卡里。

### 2. 门禁检查（硬门，不过不生图）

Read 共享 `design/{gdd,prd,interaction}.md`、项目根 `style.md` 基线与当前方案 `ui/<scheme>/style.md`：

- 三锚文档任一缺失或 `status` 非 `approved`，且无 `design/waiver.md` → **停**，指回 `ui-anchor`，不生图。
- 项目基线或方案 style.md 不存在 → **停**，指回 `ui` 总控补契约。
- 凭 waiver 放行时，向画师明示「本次凭 waiver 跳过策划门禁」。

### 3. 定 screen-id

当前方案 `ui/<scheme>/screens/screen-map.md` 存在且 approved → 从其页面清单表取 `screen-id`
...
Read `projects/<slug>/ui/<scheme>/screens/<screen-id>.md`：
...
prompt 组装顺序：项目根 style.md 基线 → 当前方案 style.md → brief → 反向限制，
写入临时文件后提交：

```bash
uv run python -m character_workflow submit-screen \
  --project <slug> --scheme <scheme-id> --screen <screen-id> --prompt-file <tmp.txt> \
...
```bash
uv run python -m character_workflow submit-screen \
  --project <slug> --scheme <scheme-id> --screen <screen-id> --prompt-file <tmp-候选N.txt> \
...
uv run python -m character_workflow set-screen-canonical \
  --project <slug> --scheme <scheme-id> --screen <screen-id> --path <选定图路径>
```
...
当前方案 `style.md` 的 `ui.typography` / `ui.geometry` / `ui.states` 三节（格式见
