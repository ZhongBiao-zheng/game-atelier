...
`shared_at` / `updated_at` / `submitted_at` / `created_at` 都是 ISO-8601 字符串。本产品写 UTC、带 `+00:00` 与微秒（如 `2026-09-25T02:13:45.123456+00:00`）。判断来源是否更新时按时刻比：`Z`、`+00:00`、`+08:00` 都接受，不带时区的当 UTC；解析不了的当「未知」（本产品不据此提示更新）。列表排序与更新判定都按时刻比，解析不了的排在最后；写方建议统一写 UTC。
