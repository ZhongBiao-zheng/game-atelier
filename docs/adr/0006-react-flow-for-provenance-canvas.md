...
交互层只渲染可见节点，并用 Domain Node 身份和选中态稳定 React Flow Node 与节点组件；图片使用浏览器原生懒加载，视频和音频接近视区后才绑定资源地址。触控板约定为双指平移、捏合缩放，鼠标左键拖框选择；触屏保留单指平移与双指缩放。窄屏检查器承载节点设置与生成草稿，宽屏生成设置跟随节点；媒体预览使用受控 Dialog，不把预览状态写入领域文档。以上均是机械层与呈现层约束，不改变 Canvas Document 或 Job 快照语义。
