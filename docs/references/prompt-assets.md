...
画师在 Atelier「创作资产」里存的提示词模板（标题 / 标签 / 带变量的正文）
...
| 步骤 | CLI | MCP | 回什么 |
| --- | --- | --- | --- |
| 1 索引 | `list-prompt-assets --tag <标签> [--query 标题子串] [--project <id>]` | `workshop_list_prompt_assets {tags, query?, project_id?, limit}` | 命中的 id / 标题 / 标签 / 最近使用 + 全库 `tag_facets`（标签与数量） |
| 2 全文 | `read-prompt-asset <asset_id>` | `workshop_read_prompt_asset {asset_id, project_id?}` | segments、variables、按默认值渲染的 prompt |
...
- 配置优先级：画师本轮明说的 > 画布 / 项目默认。确认卡里写出「配置来自哪一层」
