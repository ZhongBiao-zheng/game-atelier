...
版本与引用节点在同一把锁、同一次提交里落盘（revision 只 +1；撞 revision 不留孤儿版本）；
不接受相对路径、目录、URL。**边界**：按符号链接终点判，只允许用户家目录与工作区（data_root）内的
文件，工作区里其他画布的目录不算——画布授权是按项目给的，不能借它读走本机任意图片。越界返回
`REFERENCE_NOT_ALLOWED` 403。
